...
class Generator_Controller extends Generator_Base
{
  ...
  {
    $filename = Inflector::underscore($args[0]);
    $vars = array(
      'filename' => $filename,
      'Class'    => Inflector::camelize($args[0]),
    );
    
    $this->create_directory('/app/controllers/');
    $this->create_file_from_template(
      "/app/controllers/{$filename}_controller.php",
    ...
    $this->create_file_from_template(
      "/test/functional/{$filename}_test.php",
      '/controller/test.php',
      &$vars
    );
    ...
    $this->create_file_from_template(
      "/app/views/layouts/{$filename}.html.tpl",
      '/controller/layout.html.tpl',
      &$vars
    );
  }
}
