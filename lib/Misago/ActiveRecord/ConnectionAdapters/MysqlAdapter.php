...
#    'boolean'        => array('name' => 'tinyint', 'limit' => 1),
  ...
  function connect()
  {
    ...
    if ($this->link === false) {
      throw new ActiveRecord\ConnectionNotEstablished("Unable to connect to MySQL database.");
    }
  }
  ...
  function & columns($table)
  {
    ...
    if (!$results) {
      throw new ActiveRecord\StatementInvalid("No such table: $table");
    }
  ...
  function select_database($database=null)
  {
    ...
    if (!$success) {
      throw new ActiveRecord\StatementInvalid("Can't select database $database.");
    }
    return $success;
  }
